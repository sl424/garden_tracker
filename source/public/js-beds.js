...
var input = ['name','area'];
var ph = ['name', 'area-in2' ];
var type = ['string', 'number'];

var newhead = ['name','area','action'];
var fhead = ['name', 'area'];
var update = ['u-name', 'u-area'];
var body = ['id','name','area'];
var search = ['s-name', 's-area'];
var numcols = 3;
//
...
function searchButton(e){
	...
	req.open('POST', "http://52.36.73.75:3000/beds/all", true);
...
function bindButton(){
	var req = new XMLHttpRequest();
	//var payload = {};
	req.open("GET", "http://52.36.73.75:3000/beds/all", true);
	req.setRequestHeader('Content-Type','application/json');
	req.addEventListener('load',function(){
		if ( req.status >= 200 && req.status < 400 ) {
			var xhr = JSON.parse(req.responseText);
...
function updateRow(e){
	...
	req.open('PUT', "http://52.36.73.75:3000/beds/"+payload.id, true);
	//req.open('POST', "http://httpbin.org/post", true);
	req.setRequestHeader('Content-Type','application/json');
	req.addEventListener('load',function(){
		if ( req.status >= 200 && req.status < 400 ) {
...
function removeRow(event) {
	...
	var req = new XMLHttpRequest();
	req.open('DELETE', "http://52.36.73.75:3000/beds/"+id, true);
	req.addEventListener('load',function(){
					if ( req.status >= 200 && req.status < 400 ) {
						var xhr = JSON.parse(req.responseText);
						//console.log(xhr);
...
function updateCheck(){
				...
				var g = document.getElementById('u-area');
	...
	document.getElementById('addNew').addEventListener('click', 
			function(event){
				if ( addCheck() ) {
					...
					//console.log(payload);
					req.open('POST', "http://52.36.73.75:3000/beds", true);
					//req.open('POST', "http://httpbin.org/post", true);
					req.setRequestHeader('Content-Type','application/json');
					req.addEventListener('load',function(){
						if ( req.status >= 200 && req.status < 400 ) {
