 //Full texts		id	fid		name	do_best		sunlight	water		wateri
var msg = document.getElementById('messages');
var input = ['name','avg_high', 'avg_low','water'];
var ph = ['name', 'avg_high', 'avg_low','water-in' ];
var type = ['string', 'number', 'number','number'];

var newhead = ['name','avg_high', 'avg_low','water','action'];
var fhead = ['name', 'avg_high', 'avg_low','water'];
var update = ['u-name', 'u-avg_high', 'u-avg_low','u-water'];
var body = ['id','name','avg_high', 'avg_low','water'];
var search = ['s-name', 's-avg_high', 's-avg_low','s-water'];
var numcols = 5;
//
...
function searchButton(e){
	...
	var payload = {
		...
		"avg_high":document.getElementById("s-avg_high").value,
		"avg_low":document.getElementById("s-avg_low").value,
		"water":document.getElementById("s-water").value
	};
	req.open('POST', "http://52.36.73.75:3000/month/all", true);
...
function bindButton(){
	var req = new XMLHttpRequest();
	//var payload = {};
	req.open("GET", "http://52.36.73.75:3000/month/all", true);
	req.setRequestHeader('Content-Type','application/json');
	req.addEventListener('load',function(){
		if ( req.status >= 200 && req.status < 400 ) {
			var xhr = JSON.parse(req.responseText);
...
function addRow(xhr){
	for ( var i = 0; i < xhr.results.length; i++ ) {
		...
		var avg_high = document.createElement('td');
		avg_high.textContent = xhr.results[i].avg_high;
		row.appendChild(avg_high);

		var avg_low = document.createElement('td');
		avg_low.textContent = xhr.results[i].avg_low;
		row.appendChild(avg_low);
...
function updateRow(e){
	...
	var payload = {
		...
		"avg_high":document.getElementById("u-avg_high").value,
		"avg_low":document.getElementById("u-avg_low").value,
		"water":document.getElementById("u-water").value
	};
	req.open('PUT', "http://52.36.73.75:3000/month/"+payload.id, true);
	//req.open('POST', "http://httpbin.org/post", true);
	req.setRequestHeader('Content-Type','application/json');
	req.addEventListener('load',function(){
		if ( req.status >= 200 && req.status < 400 ) {
...
function refreshRow( xhr ) {
	...
	current = current.nextSibling;
	current.textContent = xhr.results[0].avg_high;
	current = current.nextSibling;
	current.textContent = xhr.results[0].avg_low;
	current = current.nextSibling;
...
function removeRow(event) {
	...
	var req = new XMLHttpRequest();
	req.open('DELETE', "http://52.36.73.75:3000/month/"+id, true);
	req.addEventListener('load',function(){
					if ( req.status >= 200 && req.status < 400 ) {
						var xhr = JSON.parse(req.responseText);
						//console.log(xhr);
...
function updateCheck(){
				...
				var d = document.getElementById("u-avg_high");
				var e = document.getElementById("u-avg_low");
...
function addCheck(){
				...
				var d = document.getElementById("avg_high");
				var e = document.getElementById("avg_low");
			...
			function(event){
				if ( addCheck() ) {
					...
					var payload = {
						...
						"avg_high":document.getElementById("avg_high").value,
						"avg_low":document.getElementById("avg_low").value,
						"water":document.getElementById("water").value 
					};

					//console.log(payload);
					req.open('POST', "http://52.36.73.75:3000/month", true);
					//req.open('POST', "http://httpbin.org/post", true);
					req.setRequestHeader('Content-Type','application/json');
					req.addEventListener('load',function(){
						if ( req.status >= 200 && req.status < 400 ) {
