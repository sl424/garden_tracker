...
I propose a database to track the plants grown in the backyard garden. There will
be several raised beds and hundreds of seeds. I hope it can be used to
streamline planting and maximize productivity of the garden.\\
...
Entities:
\begin{itemize}
\item Beds - location, soil type, sunlight, water, size, season, 
\item Seeds - seed spacing, row spacing, cycle, sunlight, soil, water, harvest
\item Season - Month, avg high, avg low, precipitation
\item Plant Family - name, alias
\end{itemize}

Relationship:
\begin{itemize}
	\item many-to-many: seeds can be planted in any suitable beds and a bed
		can contain multiple different plants.
	\item one-to-many: each seed is from a plant family and a family can
		have multiple seeds.
	\item one-to-one: each seed is best planted in the appropriate season.
	\item many-to-many: track the history of each beds and the plant family
		to maximize crop rotation
\end{itemize}
...
%\lstinputlisting{data-def.txt}

\homeworkSection{Data Manipulation Queries}
%\lstinputlisting{data-selection.txt}
